...
class HistoryDialog(QDialog):
    def __init__(self, history, parent=None):
        ...
        self.btn_summary = QPushButton("📊 Tổng món")
        self.btn_summary.clicked.connect(self.show_summary)
        layout.addWidget(self.btn_summary)

        self.btn_per_order = QPushButton("📄 Từng đơn")
        self.btn_per_order.clicked.connect(self.show_summary_per_order)
        layout.addWidget(self.btn_per_order)
    ...
    def show_summary(self):
        ...
        QMessageBox.information(self, "Thống kê", text)

    # ===== THỐNG KÊ TỪNG ĐƠN =====
    def show_summary_per_order(self):
        dialog = QDialog(self)
        dialog.setWindowTitle("Thống kê từng đơn")
        ...
        for i, order in enumerate(self.history, 1):
            text += f"--- Đơn {i} ---\n"
            ...
            for item in self.normalize_items(order.get("items", [])):
                ...
                temp[name] = temp.get(name, 0) + qty

            for name, qty in temp.items():
                text += f"{name}: {qty}\n"
